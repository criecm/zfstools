#!/bin/sh
#
# Installe une synchro type PRA (replication stream)
#
usage() {
  echo "usage: $0 [-k sshkey] SRCHOST:ZFSVOLUME DESTZFSVOLUME" 
  echo
  echo "examples:"
...
SRC=$1
DST=$2
SSHKEY=${SSHKEY:-~/.ssh/id_ed25519_pra}
...
if ! [ -e "$SSHKEY" ]; then
  echo "Create $SSHKEY"
  ssh-keygen -t ed25519 -f $SSHKEY -N '' -C 'pra script on '$(hostname -s)
fi
...
if ssh -oBatchMode=yes -ax -oStrictHostKeyChecking=accept-new root@$SRCHOST "echo ok" | grep -q ok; then
  if ! env SSH_AUTH_SOCK='' ssh -oIdentitiesOnly=yes -oBatchMode=yes -axi $SSHKEY $SRCHOST $DSTHOST $SRCVOL connect 2>/dev/null; then
    ...
    echo "command=\"$(realpath $(dirname $0))/send_zfs_pra.sh\" $(cat $SSHKEY.pub)" | ssh root@$SRCHOST 'cat >> .ssh/authorized_keys'
  else
...
if crontab -l | grep 'sync_zfs_pra_from.*'$SRCHOST:$SRCVOL; then
  echo "Deja dans la crontab"
  exit 0
fi

echo "Lancer une premiere synchro:"
echo "$(realpath $(dirname $0))/sync_zfs_pra_from.sh $SSHKEY $SRCHOST:$SRCVOL $DST"
echo 
